...
iic_bus_t MPU_MPU_iic_instance={          //iic总线实例
      .IIC_SCL_PORT=GPIOB,
...
int8_t        my_iic_rece_multibyte       (void * bus,uint8_t * pdata,uint8_t size){
    IIC_Read_Multi_Byte(&MPU_iic_instance,NULL,NULL,size,pdata);
    return AHT_OK;                    //dev addr + reg addr + size + pdata      
}
